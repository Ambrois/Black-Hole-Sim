// To Dos
// TODO necessary features / debugging
//  - since the disk is transparent, for horizontal orbits the light gets added forever until it runs out of steps, need to add translucency
//    - this is also an optimization, as rays near disk would end earlier
//  - make the collision be detected by looking at the last 2 points and seeing if that line intersects the object
//    - or if the leading end is inside ofc
//
// TODO Optimizations
//  - cap min r, and max r for ray_pos
//    - min r based on if it's within 3M
//  - make a dynamic step size func
//    - compute step size given r, metric, and closest object distance
//  - dynamic rk order for different r/M
//  - use cos_psi directly instead of psi
//  - for cleaner code, make rotate to and from funcs so don't have to parse all that coord switching nonsense more than once
//
// TODO Later, not necessary
...
mat3 rotation_matrix(point_cart3 cam, vec_cart3 dir) {
  ...
  // just to make sure v2 is linearly indepent from v1
...
u_vec rk4_step(u_vec current_u, float h, SC_Metric metric) {
  ...
  float // use averaged sloped to find next points
...
void check_test_cube(
  ...
  // cube center
  point_cart3 pos = point_cart3(100., 60., 70.);
  float cube_size = 20.;
  float opacity = 1.;

  bool // check if leading point is inside walls of cube
    in_x = point.x <= pos.x+cube_size && point.x >= pos.x-cube_size,
    in_y = point.y <= pos.y+cube_size && point.y >= pos.y-cube_size,
    in_z = point.z <= pos.z+cube_size && point.z >= pos.z-cube_size,
...
// Just for fun let's add grid lines
//  - for now just return bool and add to existing system
//  - and design translucency
//    passthrough in [0,1] is the amount of light that passes through
//    - so need to check if inside,
//    - and add color and subtract passthrough if inside
